...
// This is a temporary bypass for when email credentials are not working
...
const transporter = isDevelopment ? {
  ...
  sendMail: async (mailOptions) => {
    ...
    console.log('Verification Link:', mailOptions.html.match(/href="([^"]+)"/)?.[1] || 'No link found');
    console.log('=== END EMAIL BYPASS ===\n');
    return { messageId: 'dev-bypass-' + Date.now() };
  }
...
export const sendVerificationEmail = async (email, token) => {
  ...
  if (isDevelopment) {
    // Development bypass - just log the link
    console.log(`\n📧 Email verification bypassed for ${email}`);
    console.log(`🔗 Verification link: ${verificationLink}\n`);
    return { success: true, message: 'Verification email bypassed in development' };
  }
...
export const sendPasswordResetEmail = async (email, token) => {
  ...
  if (isDevelopment) {
    // Development bypass - just log the link
    console.log(`\n📧 Password reset email bypassed for ${email}`);
    console.log(`🔗 Reset link: ${resetLink}\n`);
    return { success: true, message: 'Password reset email bypassed in development' };
  }
